...
# Module-level cache: re-mmap'ing the same multi-GB safetensors file on every
# CrossEncoderReranker() instantiation is wasteful and crashes (access violation)
# on Windows torch/safetensors after a few repeated loads. Load each model once per process.
...
class FlashrankReranker:
    ...
    def rerank(self, query: str, documents: list[dict], top_k: int = RERANK_TOP_K) -> list[RerankResult]:
        ...
        return [
            RerankResult(text=r["text"], original_score=0.0, rerank_score=float(r["score"]),
                         metadata=r.get("meta", {}), rank=i)
